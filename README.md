...
- **Analitik Visual (📊 Analitik)** — Grafik breakdown Funnel CRM status, Top Kategori, Kelengkapan Kontak, dan Distribusi Rating.
- **Multi-select & Batch Actions** — Pilih beberapa baris (checkbox) untuk ubah status massal, hapus massal, atau export terpilih.
- **Quick Detail Drawer** — Klik nama tempat untuk membuka panel slide-over dengan tombol cepat WhatsApp, Telepon, Google Maps, editor Catatan & Tag kustom.
...
- **Pagination & Sorting Header** — 25 / 50 / 100 / 250 baris per halaman. Klik header kolom (Nama, Rating, Kategori, Status) untuk mengurutkan data.
- **Statistik & Filter Lengkap** — Total tempat, telepon, website, email, sosial, rating, status CRM.
- **Export Serbaguna** — Export ke **CSV, Excel (.xls), JSON, TSV**, serta tombol khusus **Salin Semua Email** & **Salin Semua Telepon**.
...
`name`, `category`, `rating`, `reviews`, `address`, `phone`, `website`, `hasWebsite`, `email`, `emailsAll`, `socials`, `hours`, `plusCode`, `lat`, `lng`, `status`, `note`, `tags`, `url`
...
| `background.js` | Service worker: state machine crawl, navigasi tab, simpan hasil, webhook, deduplikasi |
| `popup.html/css/js` | Kontrol + progress + export |
| `dashboard.html/css/js` | Tabel/kartu, pagination, filter, visual analytics, batch bar, detail drawer, statistik |
| `email.js` | Pencari email & sosial — disuntikkan ke website tempat, bukan ke Maps |
| `export.js` | Helper CSV/TSV/Excel/JSON dipakai bersama popup & dashboard |
...
104 test: parsing angka (id-ID & en-US), ekstraksi koordinat, kunci dedup, merge lintas sesi, ekstraksi sosial media, helper copy massal email/telepon, format export Excel, escaping CSV, aritmetika pagination, serta kekekalan anotasi manual saat crawl ulang.
